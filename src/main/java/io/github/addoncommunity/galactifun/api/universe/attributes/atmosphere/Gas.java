...
public enum Gas {
    ...
    AMMONIA("c7a1ece691ad28d17bbbcecb22270c85e1c9581485806264c676de67c272e2d0", CoreRecipeType.CHEMICAL_REACTOR, new ItemStack[] {
            NITROGEN.item.clone(), HYDROGEN.item.asQuantity(3), null,
            null, null, null,
            null, null, null
    }),
    OTHER;
    ...
    static {
        if (SlimefunItems.FREEZER_2.getItem() instanceof Freezer freezer) {
            freezer.registerRecipe(10, new ItemStack[] { NITROGEN.item().clone() }, new ItemStack[] { SlimefunItems.REACTOR_COOLANT_CELL.asQuantity(4) });
        }
        if (SlimefunItems.FREEZER_3.getItem() instanceof  Freezer freezer) {
            freezer.registerRecipe(7, new ItemStack[] { NITROGEN.item().clone() }, new ItemStack[] { SlimefunItems.REACTOR_COOLANT_CELL.asQuantity(4) });
        }

        if (SlimefunItems.COMBUSTION_REACTOR.getItem() instanceof CombustionGenerator generator) {
            generator.registerFuel(new MachineFuel(15, HYDROGEN.item().clone()));
            generator.registerFuel(new MachineFuel(30, HYDROCARBONS.item().clone()));
            generator.registerFuel(new MachineFuel(70, AMMONIA.item().clone()));
            generator.registerFuel(new MachineFuel(200, METHANE.item().clone()));
        }

        if (BaseItems.DIAMOND_ANVIL.getItem() instanceof DiamondAnvil anvil) {
            anvil.registerRecipe(10, new ItemStack[] { HYDROGEN.item().asQuantity(4) }, new ItemStack[] { HELIUM.item().clone() });
            anvil.registerRecipe(10, new ItemStack[] { HELIUM.item().asQuantity(4) }, new ItemStack[] { BaseMats.FUSION_PELLET.clone() });
        }
    }
