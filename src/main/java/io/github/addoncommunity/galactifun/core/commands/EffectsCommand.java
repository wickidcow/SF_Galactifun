...
public final class EffectsCommand extends SubCommand {
    ...
    public void execute(@Nonnull CommandSender commandSender, @Nonnull String[] strings) {
        ...
        if (world == null) {
            Messages.red(p, "You must be in a Galactifun world to execute this command");
            return;
        }

        for (Map.Entry<AtmosphericEffect, Integer> entry : Galactifun.protectionManager().getEffectsAt(p).entrySet()) {
            Messages.yellow(p, String.format("Effect: %s, Level: %d",
